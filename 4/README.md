...
첫 번째 요청 → 정상 처리 (API 서버로 전달)

두 번째 요청 → 정상 처리 (API 서버로 전달)

세 번째 요청 → 차단 (HTTP 429 Too Many Requests 응답 반환)
...
큐가 가득 차면 새로운 요청을 거부 (429 Too Many Requests 응답)

고정된 속도로 요청을 하나씩 처리 (균일한 부하 유지)

서버 과부하를 방지하면서 일정한 속도로 요청을 처리
